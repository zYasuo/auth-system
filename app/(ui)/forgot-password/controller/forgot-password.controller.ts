...
export class ForgotPasswordController {
  ...
  async requestPasswordReset(formData: FormData): Promise<TActionState> {
    try {
      ...
      return await this.passwordResetService.requestPasswordReset(validationResult.data.email)
    } catch (error) {
  ...
  private validateFormData(
    data: any,
  ): { success: true; data: { email: string } } | { success: false; error: TActionState } {
